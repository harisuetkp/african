...
export default function Landing() {
  ...
  const handleGetStarted = async () => {
    ...
    window.location.href = createPageUrl('login') + `?from_url=${encodeURIComponent(window.location.origin + nextUrl)}`;
  };
              ...
              <div className="flex justify-center gap-2 mt-6">
                {testimonials.map((_, idx) => (
                  <button
                    ...
                    className={`h-2 rounded-full transition-all ${idx === activeTestimonial ? 'bg-purple-600 w-8' : 'bg-gray-300 w-2'
                      }`}
                  />
                ))}
              </div>
